...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const bottomNavItems = [
    ...
    ...(!isAdmin ? [{ name: 'Plans', path: '/plans', icon: 'M21 7.5l-9-5.25L3 7.5m18 0l-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9' }] : []),
      ...
      <header className="sticky top-0 z-40 h-16 flex items-center justify-between px-4 sm:px-6 bg-[#fbfbfd]/80 backdrop-blur-xl border-b border-[#e6e6eb]">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[15px] font-semibold tracking-tight">
            {isAdmin ? <>prime<span className={accentText}>admin</span>pro</> : <>prime<span className={accentText}>invest</span>pro</>}
          </span>
        </div>
